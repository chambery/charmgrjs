...
function recalc_equipment_page() {
	...
	for(var i in chardata.equipment) {
		var html = "<tr><td colspan='5'><table style='width: 100%;border: 1px solid #D0D0D0' rules='cols'><tr><td colspan='2'><span style='float: right;color: #D0D0D0'><a class='fake_link' onclick=\"add_bene(" + i + ")\" onhover=''>edit</a> &nbsp;<a id='delete_equipment_" + chardata.equipment[i].id + "' class='fake_link' onclick='delete_bene(" + i + ")'>delete</a></span>" + chardata.equipment[i].name + "</td></tr>";
...
function delete_bene(index) {
	remove(chardata.equipment, index);
	save_character();
...
function add_bene(index) {
	...
	dialog_html += "<tr><td>Benefit:</td><td>" + type + "</td><td id='benefit_detail'>" + create_select('benefit_detail', skills.get(), '', false, "style='width: 55px'") + "</td><td>Bonus:</td><td><input id='benefit_mod' class='two_digit' type='text'/></td>";
	dialog_html += "<td style='text-align: right' nowrap><a class='box btn' style='width: 30px;' onclick=\"add_benefit($('select[id=benefit_detail]').val(), $('select[id=benefit_detail] option:selected').text(), $('#benefit_mod').val() )\">add&darr;</a></td></tr>";
	dialog_html += "<tr><td colspan='3' valign='top'>Notes:</td><td rowspan='2' colspan='3' valign='top' style='border: 1px solid #D0D0D0;'><div id='equipment_benefits' >" + create_benefits_table(equipment, index) + "</div></td></tr>";
	dialog_html += "<tr><td colspan='3' valign='top'><textarea id='equipment_notes' style='width: 95%'>" + (equipment ? equipment.notes : '') + "</textarea></td></tr></table>";
