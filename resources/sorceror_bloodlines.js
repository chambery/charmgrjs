...
var bloodline_powers = new TAFFY([{
	...
	levels: {
		2: {
			dr: function(dr) {
				dr["electricity"] = (dr["electricity"] ? dr["electricity"] : 0) + 5;
				
			},
			save: function(save) {
				save["poison"] = (save["poison"] ? save["poison"] : 0) + 2;
			},
			vals: ["5", "2"]
		},
		8: {
			dr: function(dr) {
				dr["electricity"] = (dr["electricity"] ? dr["electricity"] : 0) + 10;
				
			},
			save: function(save) {
				save["poison"] = (save["poison"] ? save["poison"] : 0) + 4;
			},
			vals: ["10", "4"]
		}
	},
	...
	levels: {
		19: {
			dr: function(dr) {
				dr["paralysis"] = -1;
				dr["sleep"] = -1;
				if(chardata.draconic_energy_type) {
					dr[chardata.draconic_energy_type] = -1;
				}
			}
		}
	},
		...
		2: {
			dr: function(dr) {
				if(chardata.bloodline_energy_type) {
					dr[chardata.bloodline_energy_type] = (dr[chardata.bloodline_energy_type] ? dr[chardata.bloodline_energy_type] : 0) + 10;
				}
			}
		}
	},
