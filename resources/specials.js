var specials = TAFFY([
{
	...
	op: "if ($(this).attr('checked')) {var level = calc_level(chardata.xp);var clazz = classes({name : chardata.class_name}).first();var size = races({name : chardata.race_name}).first().size;var temp = clazz.base_attack_bonus[level];var base_attack_bonus = temp.split('/')[0];var flurry_attack = clazz.flurry_of_blows[level];var attacks = flurry_attack.split('/');var flurry_atts = '';for ( var j in attacks) {flurry_atts += pos(parseInt(attacks[j]) + parseInt(base_attack_bonus));flurry_atts += (parseInt(j) + 1 < attacks.length ? '/' : '');}for ( var i in chardata.weapons) {var weapon = weapons({id : chardata.weapons[i].weapon_id}).first();if (jQuery.inArray(weapon.id, clazz.flurry_weapons) > -1) {var att = $('#weapon_' + i + '_att').text();$('#weapon_' + i + '_att').text(calc_attack(flurry_atts, weapon, chardata.weapons[i]));if (weapon.id == 60 || weapon.id == 61) {var damage = clazz.flurry_damage[level][size];$('#weapon_' + i + '_dam').text(damage + pos(calc_ability_modifier(chardata.abilities[0])));}}}} else {recalc_main_page();}"
}, {
	name: "Unarmed Strike",
