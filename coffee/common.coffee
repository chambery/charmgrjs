...
calc_total_base_feats_count = (race_name, xp) ->
	...
	for classname of chardata.classes
		...
		while i <= chardata.classes[classname].level
			base_feat_count += (if base_feats.indexOf(i) > -1 then 1 else 0)
			i++
	base_feat_count

calc_total_class_feats_count = (xp) ->
	class_feat_count = 0
	for classname of chardata.classes
		class_feats = classes.first(name: classname).bonus_feats_levels
		i = 0
		
		while i <= chardata.classes[classname].level
			class_feat_count += (if class_feats.indexOf(i) > -1 then 1 else 0)
			i++
	class_feat_count
...
calc_skill_mod = (skill, skill_ability_score, acp, subtype) ->
	acp = acp | 0
	char_skill_points = 0
	char_skill = false
...
calc_ability_modifier = (score) ->
	Math.ceil((score - 11) / 2) | 0

set_links_part = (page_id) ->
	classes_html = ""
...
switch_content = (view_id, chardata) ->
	if view_id == 0
		main.do_main()
	else if view_id == 1
		main.do_edit()
...
populate_object = (obj, name, value) ->
	obj[name] = value
	obj
...
create_select = (name, items, onchange_action, include_empty, extra_data, category_filter, selected_value, id_as_value) ->
...
create_select1 = (name, items, onchange_action, selected_value, extra_data) ->
...
calc_will = (wis_score, class_name, xp, char_feats) ->
...
calc_armor_bonus = (char_armor, db, dammit) ->
	max_dex_bonus = null
	armor_bonus = 0
	if char_armor?
		for i of char_armor
...
calc_damage = (weapon, char_feats, char_weapon) ->
	damages = []
...
show_dialog = (title, content, save_on_close, close_fn, opts) ->
...
show_item_detail = (table, obj_id, modify_detail) ->
	obj = null
...
reconstitute_array = (array_obj, attr_name) ->
	array = null
	if array_obj
		array = []
		for i of array_obj
...
create_selector_grid = (items, anchor_table, click_fn, populate_data, cols, use_text_link) ->
	cols = 2	unless cols?
...
create_selector_group = (item, title, all_options, option_db, cols, click_fn) ->
...
calc_attack = (base_attack_bonuses, weapon, char_weapon, other_mod) ->
	attack_override = 0
...
calc_base_attack = (base_attack_bonuses, ability_score, other_mod) ->
...
do_class_functions = (page, location, obj) ->
