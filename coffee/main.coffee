...
main.load = ->
  ...
  if players_companion.last_character
    ...
    chardata.options = (if chardata.options then chardata.options else {})
    load_static_data()
...
main.build_main_page = ->
  ...
  chardata.skills = new TAFFY([])  unless chardata.skills?
  ...
  $("#specials").hide()
  chardata.weapons = []  unless chardata.weapons
  ...
  html = []
  for j of chardata.armors
    armor_data = armors.first(name: chardata.armors[j]["armor_name"])
  ...
  for j of chardata.shields
    shield_data = shields.first(_id: chardata.shields[j]["shield_id"])
  ...
  char_domains = []
  for domain of chardata.domains
    domain = domains.first(name: chardata.domains[domain])
  ...
  for classname of chardata.classes
    clazz = classes.first(name: classname)
    chardata.classes[classname].spells = [ [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [] ]  if clazz.spells_known and not chardata.classes[classname].spells
    all_spells = $.merge([], (if clazz.spells_known then chardata.classes[classname].spells else clazz.spells))
...
main.print_spells = (spell_list, clazz, spells_html, level, char_domains) ->
  spells_printed = 0
  for i of spell_list
    if not clazz.spells_known or chardata.classes[classname].spells[level].indexOf(spell_list[i]) > -1
      spell = spells.first(name: spell_list[i])
      for j of spell.descriptors
        return  if spell.descriptors[j] != chardata.alignment and goodness.find(spell.descriptors[j]) != chardata.goodness
...
main.populate_main_page = ->
  ...
  $("input[id='hp']").val (if not chardata.hp? or chardata.hp.length <= 0 then 0 else chardata.hp)
  ...
  for j of chardata.armors
    armor_data = armors.first(name: chardata.armors[j]["armor_name"])
    $("#armor_" + j + "_name").text (if chardata.armors[j].name? then chardata.armors[j].name + (if chardata.armors[j].name.indexOf(armor_data.name) == -1 then " (" + armor_data.name + ")" else "") else armor_data.name)
    $("#armor_" + j + "_bon").text armor_data.bon
    $("#armor_" + j + "_acp").text armor_data.acp
    $("#armor_" + j + "_note").text chardata.armors[j]["notes"]
    $("tr[id='armor_" + j + "_note']").toggle chardata.armors[j].note? and chardata.armors[j].note.length > 0
  for j of chardata.shields
    shield_data = shields.first(name: chardata.shields[j]["shield_name"])
    $("#shield_" + j + "_name").text (if chardata.shields[j].name? then chardata.shields[j].name + (if chardata.shields[j].name.indexOf(shield_data.name) == -1 then " (" + shield_data.name + ")" else "") else shield_data.name)
    $("#shield_" + j + "_bon").text shield_data.bon
    $("#shield_" + j + "_acp").text shield_data.acp
    $("#shield_" + j + "_note").text chardata.shields[j]["notes"]
    $("tr[id='shield_" + j + "_note']").toggle chardata.shields[j].note? and chardata.shields[j].note.length > 0
