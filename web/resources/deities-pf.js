var deities, get_deities_for_alignment;

get_deities_for_alignment = function(align, good) {
  var align_idx, align_range, criteria, good_idx, goodness_range;
  criteria = {};
  if (align && good) {
    align_range = [align];
    goodness_range = [good];
    good_idx = goodness.indexOf(good);
    align_idx = alignments.indexOf(align);
    if (align_idx + 1 < 3) align_range.push(alignments[align_idx + 1]);
    if (align_idx - 1 > -1) align_range.push(alignments[align_idx - 1]);
    if (good_idx + 1 < 3) goodness_range.push(goodness[good_idx + 1]);
    if (good_idx - 1 > -1) goodness_range.push(goodness[good_idx - 1]);
    criteria = {
      alignment: align_range,
      goodness: goodness_range
    };
  }
