...
main.build_main_page = function() {
  ...
  while (i < char_feats.length) {
    ...
    if (feat && feat.conditional) conditional_feats.push(feat);
    i++;
  }
  ...
  $("#specials").hide();
  if (!window.chardata.weapons) window.chardata.weapons = [];
...
main.print_spells = function(spell_list, clazz, spells_html, level, char_domains) {
  ...
  for (i in spell_list) {
    if (!clazz.spells_known || window.chardata.classes[classname].spells[level].indexOf(spell_list[i]) > -1) {
      ...
      if (spells_printed % 2 === 1) spells_html.push("</tr><tr>");
      spells_printed++;
    }
  }
  return spells_html;
...
main.populate_main_page = function() {
  ...
  $("#dr").append("</tr>");
  if (!$("#dr_" + row_cnt).children().length) $("#dr_" + row_cnt).remove();
  if (!col_cnt) $("#dr").hide();
  ...
  $("#saves").append("</tr>");
  if (!$("#save_" + row_cnt).children().length) $("#save_" + row_cnt).remove();
  if (!col_cnt) return $("#saves").hide();
};
...
main.populate_skill_entry = function(skill, acp, subtype) {
  ...
  subtype_selector = "";
  if (subtype) subtype_selector = "][subtype='" + subtype + "'";
