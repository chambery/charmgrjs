	...
	for (var classname in chardata.classes) {
		if (spellcasters.indexOf(classname) > -1) {
			...
			if (clazz.spells_known.length > 0) {
				$('#content').append(
						"<div id='spell_selections_" + classname + "'><table id='spell_selection_" + classname
								+ "' style='width: 100%' border='0' margin='0'></table></div>");
				...
				for ( var spell_level in clazz.spells_known[chardata.classes[classname].level]) {
					if (clazz.spells_known[chardata.classes[classname].level][spell_level] != '-') {
						...
						$("table#spell_selection_" + classname).append(
								"<tr id='" + classname + "_level_" + spell_level + "' style='font-size: x-small; background-color: #8DC3E9'><td colspan=3 ><span id='" + classname + "_spells_remaining_" + spell_level
										+ "' style='float: right;'>" + spell_selections_remaining + " </span>Level " + spell_level + "</td></tr>");
						...
						for ( var j in clazz.spells[spell_level].sort()) {
							...
							$("tr#" + classname + "_level_" + spell_level + "_sub_" + ((j / 3) | 0)).append(
									"<td valign='top'><input id='" + spell._id + "_" + classname + "' type='checkbox'/><a id='spell_" + spell._id + "' class='fake_link'>" + spell.name + "</a></td>");
...
function recalc_spells_page(level, spell_name, classname) {
	...
	if ($("input#" + spell_id + "_" + classname).attr('checked')) {
		// add the missing spell level array if necessary
		var missing_spell_levels = (level + 1) - chardata.classes[classname].spells.length;
		if (missing_spell_levels > 0) {
			for (i = 0; i < missing_spell_levels; i++) {
				chardata.classes[classname].spells.push([]);
			}
		}
