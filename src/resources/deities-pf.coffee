get_deities_for_alignment = (align, good) ->
	criteria = {}
	if align and good
		align_range = [ align ]
		goodness_range = [ good ]
		...
		align_range.push alignments[align_idx + 1]	if align_idx + 1 < 3
		align_range.push alignments[align_idx - 1]	if align_idx - 1 > -1
		goodness_range.push goodness[good_idx + 1]	if good_idx + 1 < 3
		goodness_range.push goodness[good_idx - 1]	if good_idx - 1 > -1
		criteria =
			alignment: align_range
			goodness: goodness_range
	deities( criteria ).get()
deities = TAFFY([
	name: "Erastil"
	alignment: "Lawful"
	goodness: "Good"
