var abilities, alignments, armor_edit_data, calc_ability_modifier, calc_ability_score, calc_ac, calc_armor_acp, calc_armor_bonus, calc_attack, calc_base_attack, calc_base_attack_bonus, calc_cmb, calc_cmd, calc_current_level, calc_damage, calc_dr, calc_equip_mod, calc_flat_footed_ac, calc_fort, calc_init, calc_level, calc_ranks, calc_ref, calc_save, calc_shield_acp, calc_size_mod, calc_skill_mod, calc_spell_resistance, calc_synergies, calc_total_base_feats_count, calc_total_class_feats_count, calc_touch_ac, calc_will, capitalize, char_classes, count_attrs, create_new_character, create_select, create_select1, create_selector_grid, create_selector_group, curr_xp, damage_reductions, dehtmlize, do_class_functions, draconic_types, equipment_benefits, exotic_weapons, export_character, generate_id, get_all_char_feats, get_all_characters, get_char_feats, get_class_feat_names, get_class_feats, get_special_abilities, goodness, grapple_size_mod, htmlize, intersection, is_class_feat, is_class_skill, is_empty, is_number, klone, loaded_static_data_tags, martial_weapons, natural_spellcasters, options, parse_character_data, players_companion, populate_object, pos, reconstitute_array, remove, save_against, set_links_part, shield_edit_data, show_detail_ignore, show_dialog, show_item_detail, simple_weapons, sizes, spellcasters, spellpickers, switch_content, toggle_visible, update_options, update_weapon, weapon_edit_data;
update_weapon = function(name, index) {
  var weapon;
...
calc_total_base_feats_count = function(race_name, xp) {
  var base_feat_count, base_feats, classname, i;
  base_feat_count = (race_name === "Human" ? 1 : 0);
  base_feats = [0, 2, 5, 8, 11, 14, 17];
  for (classname in chardata.classes) {
    i = 0;
    while (i <= chardata.classes[classname].level) {
...
calc_total_class_feats_count = function(xp) {
  var class_feat_count, class_feats, classname, i;
  class_feat_count = 0;
  for (classname in chardata.classes) {
    class_feats = classes.first({
      name: classname
    }).bonus_feats_levels;
    i = 0;
    while (i <= chardata.classes[classname].level) {
...
calc_armor_acp = function(char_armors) {
  var acp, armor, i;
  acp = 0;
...
calc_shield_acp = function(char_shields) {
  var acp, i, shield;
  acp = 0;
...
calc_ranks = function(char_skill_points, skill, subtype) {
...
calc_ability_score = function(ability) {
...
switch_content = function(view_id, chardata) {
  if (view_id === 0) {
    return main.do_main();
  } else if (view_id === 1) {
    return main.do_edit();
    ...
    if (view_id === 4) {
      return main.do_equipment();
    }
  }
};
populate_object = function(obj, name, value) {
  obj[name] = value;
  return obj;
};
count_attrs = function(obj) {
  var count, k;
  count = 0;
  for (k in obj) {
...
create_select = function(name, items, onchange_action, include_empty, extra_data, category_filter, selected_value, id_as_value) {
  var html, i, selected;
...
create_select1 = function(name, items, onchange_action, selected_value, extra_data) {
  var html, i, selected;
...
calc_will = function(wis_score, class_name, xp, char_feats) {
...
calc_armor_bonus = function(char_armor, db, dammit) {
...
calc_damage = function(weapon, char_feats, char_weapon) {
...
show_dialog = function(title, content, save_on_close, close_fn, opts) {
  var options;
...
show_item_detail = function(table, obj_id, modify_detail) {
...
parse_character_data = function(data) {
...
reconstitute_array = function(array_obj, attr_name) {
  var array, i;
  array = null;
  if (array_obj) {
    array = [];
...
update_options = function(message) {
...
is_class_feat = function(feat_name) {
  return get_class_feat_names().indexOf(feat_name) > -1;
};
get_class_feat_names = function() {
...
get_special_abilities = function() {
...
create_selector_grid = function(items, anchor_table, click_fn, populate_data, cols, use_text_link) {
...
create_selector_group = function(item, title, all_options, option_db, cols, click_fn) {
...
calc_attack = function(base_attack_bonuses, weapon, char_weapon, other_mod) {
...
calc_base_attack = function(base_attack_bonuses, ability_score, other_mod) {
...
do_class_functions = function(page, location, obj) {
...
calc_touch_ac = function(dex_score, race_name, char_feats) {
...
calc_base_attack_bonus = function() {
...
weapon_edit_data = ["name", "att", "dam", "crit", "note"];
options = new TAFFY([
  {
    name: "owner",
    description: "Owner",
